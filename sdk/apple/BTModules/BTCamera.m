...
- (void)setup {
    ...
    [BTApp handleCommand:@"BTCamera.show" handler:^(id params, BTCallback callback) {
        [self _showCamera:params callback:callback];
    }];
    
    [BTApp handleCommand:@"BTCamera.hide" handler:^(id data, BTCallback responseCallback) {
        if (!picker) { return; }
    ...
    [BTApp handleCommand:@"BTCamera.capture" handler:^(id params, BTCallback callback) {
    ...
    [BTApp handleCommand:@"BTCamera.stopCapture" handler:^(id params, BTCallback callback) {
...
//        [BTApp.webView.superview insertSubview:picker.view belowSubview:app.webView];
//        callback(nil,nil);
    } else {
        [BTApp.instance.window.rootViewController presentViewController:picker animated:YES completion:NULL];
        captureParams = data;
        captureCallback = callback;
    }
}
...
- (void) _error:(id)error response:(id)response {
    if (captureParams[@"position"]) {
        ...
        [BTApp.instance.window.rootViewController dismissViewControllerAnimated:YES completion:^{
            captureCallback(nil, response);
            picker = nil;
        }];
    }
...
- (void)imagePickerControllerDidCancel:(UIImagePickerController *)thePicker {
    if (captureParams && !captureParams[@"position"]) {
        [BTApp.instance.window.rootViewController dismissViewControllerAnimated:YES completion:NULL];
    }
    [BTApp notify:@"BTCamera.imagePickerControllerDidCancel"];
    picker = nil;
}
@end
