...
- (void) setup {
    ...
    [BTApp handleCommand:@"BTFiles.writeJson" handler:^(id data, BTCallback responseCallback) {
        [self _writeJson:[BTFiles path:data] jsonValue:[data objectForKey:@"jsonValue"] andRespond:responseCallback];
    }];
    [BTApp handleCommand:@"BTFiles.readJson" handler:^(id data, BTCallback responseCallback) {
        [self _readJson:[BTFiles path:data] andRespond:responseCallback];
    }];
    [BTApp handleCommand:@"BTFiles.clearAll" handler:^(id data, BTCallback responseCallback) {
        [self _clearAll:data responseCallback:responseCallback];
    }];
    [BTApp handleRequests:@"BTFiles.read" handler:^(NSDictionary *params, WVPResponse *response) {
        [response respondWithData:[self read:params] mimeType:params[@"mimeType"]];
    }];
    [BTApp handleCommand:@"BTFiles.fetch" handler:^(id params, BTCallback callback) {
